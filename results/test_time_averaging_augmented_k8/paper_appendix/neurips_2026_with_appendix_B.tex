...
eigval-scaling off, eigenvector cache size $15$. Software stack:
PyTorch $2.10$, CUDA $12.8$, conda environment \texttt{schnet-angle}.
Standard deviations reported below are sample standard deviations
(\texttt{statistics.stdev}, $\mathrm{ddof}\!=\!1$, dividing by $n-1$);
with $n\!=\!3$ this is approximately $\sqrt{n/(n-1)}\!\approx\!1.22\times$
the population std.
...
\texttt{random\_augmented} matches or outperforms both canonization
arms (\texttt{map}, \texttt{oap}) at $8$ of $9$ tested $(k,h)$ cells.
The exception is $k\!=\!3$/$h\!=\!512$, where \texttt{oap} edges
\texttt{random\_augmented} by $0.0036$~AP. The \textsc{aug-$K$} column
reports test-time averaging over $K$ ambiguity-group draws (see text).
Best entry per row in \textbf{bold}.}
...
sample standard deviations over $3$ seeds. The \texttt{random\_augmented}
margin over the better canonization arm exceeds the seed std on both
sides at every hidden dim.}
\label{tab:lpe_molpcba_k16}
\begin{tabular}{rccc}
\toprule
$h$  & \texttt{map}              & \texttt{oap}              & \texttt{random\_augmented}     \\
\midrule
 16  & $0.0610 \pm 0.0009$       & $0.0660 \pm 0.0054$       & $\mathbf{0.0691 \pm 0.0008}$       \\
128  & $0.1779 \pm 0.0065$       & $0.1834 \pm 0.0036$       & $\mathbf{0.1916 \pm 0.0009}$       \\
512  & $0.2381 \pm 0.0147$       & $0.2397 \pm 0.0061$       & $\mathbf{0.2483 \pm 0.0026}$       \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Test-time averaging is essentially a no-op.}
Across all nine $(k,h)$ cells the difference between single-pass
\texttt{random\_augmented} and \textsc{aug-$K$} (Table~\ref{tab:lpe_molpcba_main},
right-most two columns) lies in $[-0.0002, +0.0005]$~AP --- below the
seed-noise floor at every cell. The training-time augmentation has
already pushed the network towards a $\G_{\mathrm{LapPE}}$-invariant
function in expectation, so explicit Reynolds-style symmetrization at
test time is essentially redundant on this benchmark. Concretely, for
the $k\!=\!16$/$h\!=\!128$ cell the network is approximately invariant
to $\sim\!65{,}000$ sign patterns despite never having been canonized.

\paragraph{Takeaways and ties to the main text.}
\begin{enumerate}\itemsep1pt
\item \emph{Augmentation matches or beats canonization at $8/9$ cells.}
      This is consistent with our Proposition~\ref{propGen} and the
      covering-number inequality~\eqref{eq:coveringIneq}: an
      augmentation-based approximation of the group-averaged predictor
      tracks the tightest of the three bounds
      ($\N(K/\G,\rho_\G,\epsilon)$), while the canonized predictor
      tracks the middle bound ($\N(c(K),\rho,\epsilon)$), achievable
      only when the canonization is sufficiently regular.
\item \emph{The single canonization win is at small $k$, large $h$.}
      The $k\!=\!3$/$h\!=\!512$ cell is the only exception. We do not
      reach a definitive interpretation; possible explanations include
      reduced effective capacity needed for the small ambiguity group
      $\{-1,+1\}^{3}$, or unmodeled regularization effects of
      \texttt{oap}'s deterministic ordering on a high-capacity model.
\item \emph{The advantage of augmentation grows with $k$ (i.e.\ with
      group size).} At $k\!=\!16$ the gap between
      \texttt{random\_augmented} and the better canonization arm
      reaches $+0.0086$~AP at $h\!=\!512$. This is qualitatively
      consistent with the covering-number
      inequality~\eqref{eq:coveringIneq}: as $|\mathcal{G}|$ grows, the
      slack between the canonization and group-averaging covering
      numbers grows, and the slack becomes easier to exhibit
      empirically.
\item \emph{Test-time symmetrization adds no measurable AP once
      training-time augmentation has converged.} This complements the
      observation from the PCA experiment of Section~\ref{sec:case_study}
      that augmentation needs more training to converge: once it has,
      the model has internalised an approximate $\G_{\mathrm{LapPE}}$
      invariance, and the explicit Reynolds operator at test time is
      redundant.
\end{enumerate}

\paragraph{Limitations.}
This experiment uses a single backbone (GIN), a single benchmark
(\texttt{ogbg-molpcba}), and three seeds per cell; no formal
statistical test is performed beyond seed standard deviations.
Other commonly used canonizations (e.g.\ Spielman, MaxAbs sign
selection, Abs / SignNet-style absolute-value features) are out of
scope here. The findings should be read as empirical evidence
complementary to the theory of Section~\ref{sec:gen} and the PCA case
study of Section~\ref{sec:case_study}, not as a benchmark claim about
the best LapPE recipe in general.
