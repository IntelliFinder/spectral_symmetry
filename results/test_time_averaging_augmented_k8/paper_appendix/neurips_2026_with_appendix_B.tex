...
covering-number inequality~\eqref{eq:coveringIneq} ---
...
We compare three strategies for resolving $\G_{\mathrm{LapPE}}$:
\begin{itemize}\itemsep0pt
\item \texttt{map}: maximum-absolute-position canonization, which fixes
      a sign by selecting the entry of largest absolute value (ties
      broken lexicographically).
\item \texttt{oap}: ordered-absolute-position canonization, which fixes
      a sign by ordering nodes by absolute position and then selecting
      the first non-zero entry. More numerically stable than \texttt{map}
      under near-ties.
\item \texttt{random\_augmented}: at every training-time forward pass,
      independently flip the sign of each eigenvector with probability
      $1/2$ before feeding the LapPE into the network. This is a
      finite-group augmentation: it approximates group averaging over
      $\G_{\mathrm{LapPE}}$ without explicitly summing over its $2^k$
      elements at each step. \emph{At evaluation time the model trained
      under \texttt{random\_augmented} is fed the deterministic
      eigsh-sign convention from the cache} (no fresh random draws),
      yielding a single deterministic prediction per graph; this is the
      number reported in the \texttt{random\_augmented} column of
      Table~\ref{tab:lpe_molpcba_main}.
\end{itemize}
...
Tables~\ref{tab:lpe_molpcba_main}--\ref{tab:lpe_molpcba_k16} are also
...
\paragraph{Takeaways and ties to the main text.}
\begin{enumerate}\itemsep1pt
\item \emph{Nominal-rank pattern: augmentation attains the row-best
      mean at $8/9$ cells.} Under matched-compute conditions
      (canonization arms also at $500$~ep at $k\!=\!16$, see Protocol
      asymmetry above), the count drops: at $k\!=\!16$ the
      \texttt{map} arm catches up at $h\!\in\!\{16,128\}$ and
      \texttt{random\_augmented} retains a nominal lead only at
      $h\!=\!512$ (Welch $p\!\approx\!0.07$, not significant at
      $n\!=\!3$). The qualitative direction --- that an
      augmentation-based approximation of group averaging is at least
      as competitive as deterministic canonization, when both are
      adequately trained --- is consistent with the covering-number
      inequality~\eqref{eq:coveringIneq} of
      Proposition~\ref{propGen}, but the appendix data does not
      independently identify the inequality's slack from training-budget
      asymmetries.
\item \emph{The lone nominal canonization win
      ($k\!=\!3$/$h\!=\!512$, \texttt{oap} by $+0.0036$~AP) is also
      within seed std} (\texttt{oap} std $\approx\!0.0022$,
      \texttt{random\_augmented} std $\approx\!0.0117$ at this cell).
      We did not run an extended-budget test at $k\!=\!3$ specifically;
      under-trained augmentation and method-specific regularization at
      large $h$ both remain viable hypotheses.
\item \emph{Nominal augmentation advantage at $h\!=\!512$ in the
      $k\!=\!16$ cell.} The largest single gap in the table, $+0.0086$
      AP at $k\!=\!16$/$h\!=\!512$, is suggestive but the trend is
      based on three $k$ values and the $k\!=\!3$ point already breaks
      monotonicity at $h\!=\!512$. We therefore do \emph{not} claim a
      monotone covering-number-driven scaling with $|\G|$ from these
      three points alone.
\item \emph{Test-time symmetrization is small but not interpreted
      here.} The \textsc{aug-$K$} column adds at most $+0.0005$~AP and
      sometimes nothing. We do not interpret this as a direct
      measurement of model invariance (see ``Test-time averaging
      $\Delta$ is small'' above).
\end{enumerate}

\paragraph{Limitations.}
\begin{itemize}\itemsep0pt
\item \emph{Three seeds.} The Welch $p$-values in
      Table~\ref{tab:lpe_molpcba_k16} are exact for $n\!=\!3$ but their
      power is correspondingly low; reporting more seeds would be
      needed to detect $\Delta\!\sim\!0.001$~AP differences with
      confidence.
\item \emph{Protocol asymmetry at $k\!=\!16$.} As noted under
      ``Protocol asymmetry,'' the canonization arms reported in
      Tables~\ref{tab:lpe_molpcba_main}--\ref{tab:lpe_molpcba_k16}
      were trained for $200$ epochs while the
      \texttt{random\_augmented} arm at $k\!=\!16$ was trained for
      $500$ epochs. Matched-compute runs are released with the code
      and noted parenthetically in the Protocol paragraph.
\item \emph{Test-time eval uses cached eigsh signs.} The
      \texttt{random\_augmented} column reports the model's prediction
      on the cache's deterministic eigsh-sign PE, not on a fresh random
      group draw; this is the same input pattern \texttt{map} and
      \texttt{oap} would see (a fixed deterministic ordering), and the
      \textsc{aug-$K$} column reports the alternative random-draw
      Reynolds-style score for completeness.
\item \emph{Multiplicity-block scope.} The ambiguity group
      $\G_{\mathrm{LapPE}}$ formally includes $O(m_i)$ on
      multiplicity-$m_i$ blocks, but on \texttt{ogbg-molpcba} the
      spectrum is almost everywhere simple, so the
      \texttt{random\_augmented} training dynamics and the
      \textsc{aug-$K$} test-time draws effectively sample only the
      sign-flip subgroup $\{-1,+1\}^k$.
\item \emph{Method scope.} Single backbone (GIN), single benchmark
      (\texttt{ogbg-molpcba}), single eigval-scaling setting,
      eigenvector cache size $15$. Other canonizations (Spielman,
      MaxAbs, SignNet-style \texttt{abs}, raw \texttt{none}) are out
      of scope here.
\item \emph{No statistical claim beyond Welch's $t$ at $n\!=\!3$.}
      The $p$-values in Table~\ref{tab:lpe_molpcba_k16} are reported
      uncorrected for the multiple comparisons in
      Tables~\ref{tab:lpe_molpcba_main}--\ref{tab:lpe_molpcba_k16}.
\end{itemize}
The findings are empirical evidence complementary to the theory of
Section~\ref{sec:gen}, not a benchmark claim about the best LapPE
recipe in general.
