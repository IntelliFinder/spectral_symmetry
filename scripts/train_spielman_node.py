"""Train distance-modulated transformer with Spielman-canonicalized eigenvector node features.

Uses DistanceTransformerClassifier + SpielmanNodeModelNet. Supports multiple
...
def main():
    parser = argparse.ArgumentParser(
        description="Train distance transformer with Spielman-canonicalized eigenvector features"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Save directory (default: results/spielman_node/{canon}_k{n_eigs})",
    )
    ...
    if args.save_dir is None:
        save_dir = Path("results") / "spielman_node" / f"{args.canonicalization}_k{args.n_eigs}"
    else:
    ...
    full_train_ds = SpielmanNodeModelNet(
        args.data_dir,
        split="train",
