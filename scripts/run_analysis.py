...
def analyze_shape(name, points, n_eigs, n_neighbors, threshold):
    ...
    try:
        result = analyze_spectrum(
            points, n_eigs=n_eigs, n_neighbors=n_neighbors, threshold=threshold
        )
        result["name"] = name
    ...
    with open(path, "w", newline="") as f:
        ...
        writer.writerow(
            [
                ...
                "eigenvalue",
                "uncanon_score",
                "is_uncanonicalizable",
                "is_uncanonicalizable_raw",
                "multiplicity",
                "spectral_gap",
            ]
        )
...
def write_summary_table(all_stats, path, threshold_used):
    ...
    with open(path, "w") as f:
        ...
        f.write(
            "| Dataset | Shapes | Avg Uncanon Ratio | Spectral Gap (mean) "
        ...
        for s in all_stats:
            f.write(
                f"| {s.dataset_name} | {s.n_shapes} | {s.avg_uncanon_ratio:.4f} "
        ...
        for s in all_stats:
            f.write(
                f"| {s.dataset_name} | {s.n_shapes} | {s.avg_repeating_eigenvalues:.2f} "
...
def main():
    ...
    parser.add_argument(
        "--n-instances", type=int, default=10, help="Instances per shape type for Symmetria"
    )
    parser.add_argument(
        "--threshold", type=float, default=None, help="Uncanon threshold (default: 5/sqrt(N))"
    )
    parser.add_argument("--n-jobs", type=int, default=1, help="Parallel workers (1=sequential)")
    parser.add_argument("--output-dir", type=str, default="results")
    parser.add_argument(
        "--data-dir", type=str, default="data", help="Root data directory for ModelNet/Symmetry3D"
    )
    parser.add_argument("--download", action="store_true", help="Download datasets if not present")
