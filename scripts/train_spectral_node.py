"""Train spectral node features model on ModelNet.

Uses canonicalized eigenvectors as node features (concatenated to xyz)
with k-NN distance-modulated attention. This tests whether spectral
information helps when provided as input features rather than attention bias.
"""
...
from src.experiments.spectral_transformer.dataset import SpectralNodeModelNet
from src.experiments.spectral_transformer.model import DistanceTransformerClassifier
...
def main():
    parser = argparse.ArgumentParser(description="Train spectral node features model on ModelNet")
    ...
    parser.add_argument("--save-dir", type=str, default="results/spectral_node", help="Save directory")
    ...
    # Datasets
    print(f"Loading ModelNet{args.variant} training set (spectral node features)...")
    train_ds = SpectralNodeModelNet(
        args.data_dir,
        split="train",
    ...
    print(f"Loading ModelNet{args.variant} test set (spectral node features)...")
    test_ds = SpectralNodeModelNet(
        args.data_dir,
        split="test",
        n_points=args.n_points,
        n_eigs=args.n_eigs,
    ...
    for epoch in range(1, args.epochs + 1):
        train_loss = train_one_epoch_dist(model, train_loader, criterion, optimizer, device)
        test_acc = evaluate_dist(model, test_loader, device)
        scheduler.step()
