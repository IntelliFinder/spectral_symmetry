...
class GCNLapPE(nn.Module):
    """GCN with Laplacian Positional Encoding for graph-level prediction.

    Same architecture as GINLapPE but uses GCNConv (single linear per layer)
    instead of GINConv (2-layer MLP), resulting in ~half the conv-layer params.
